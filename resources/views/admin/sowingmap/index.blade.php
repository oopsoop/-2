...
@section('content')
    ...
    <p>
        <a href="{{ url('admin/sowingmap/create') }}" class="btn btn-success">{{$showdata['app_add_new']}}</a>
    </p>
    <form id="" action="/admin/sowingmap/index" method="get" class="form-inline">
                    ...
                    @if ($showdata['data']->count() > 0)
                        @foreach ($showdata['data'] as $proj)
                            <tr>
                                ...
                                <td>
                                    <a href="{{ url('admin/sowingmap/update',[$proj->id]) }}" _class="btn btn-xs btn-info"><i class="fa fa-pencil text-success text" style="font-size: 15px;"></i>{{$showdata['app_edit']}}</a>
                                    {{--                                    <a href="{{ url('admin/sowingmap/chakan',[$proj->id]) }}" _class="btn btn-xs btn-info"><i class="fa fa-pencil text-success text" style="font-size: 15px;"></i>{{$showdata['app_chakan']}}</a>--}}
    ...
    <script>
          function del(id) {
              layer.confirm('您确定要删除吗？', {
                  ...
                  $.post("/admin/sowingmap/destroy/"+id,{'_method':'delete','_token':"{{csrf_token()}}"},function (data) {
                      if(data.statu==1){
                          layer.msg(data.msg);
