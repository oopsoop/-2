...
@section('content')
    ...
    <p>
        {{--<a href="{{ url('admin/wxusers/create') }}" class="btn btn-success">{{$showdata['app_add_new']}}</a>--}}
    </p>
    <form id="" action="/admin/wxusers/index" method="get" class="form-inline">
    ...
    <div class="panel panel-default">
                ...
                <tbody>
                    @if ($showdata['data']->count() > 0)
                        @foreach ($showdata['data'] as $proj)
                            <tr>
                                <td>{{ $proj->nickName }}</td>
                                <td><img src = "{{ $proj->avatarUrl }}" alt = ""></td>
        ...
        <td>
            ...
            {{--<a href="{{ url('admin/wxusers/update',[$proj->id]) }}" _class="btn btn-xs btn-info"><i class="fa fa-pencil text-success text" style="font-size: 15px;"></i>{{$showdata['app_edit']}}</a>--}}
...
{{--            <a href="{{ url('admin/wxusers/show_course',[$proj->id]) }}" _class="btn btn-xs btn-info"><i class="fa fa-pencil text-success text" style="font-size: 15px;"></i>课程详情</a>--}}
    ...
    <script>
          function del(id) {
              layer.confirm('您确定要删除吗？', {
                  ...
                  $.post("/admin/wxusers/destroy/"+id,{'_method':'delete','_token':"{{csrf_token()}}"},function (data) {
                      if(data.statu==1){
                          layer.msg(data.msg);
