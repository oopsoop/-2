...
@section('content')
    ...
    {!! Form::model($role, ['method' => 'PUT', 'id'=>'XP','route' => ['admin.roles.update', $role->id]]) !!}
	...
	<script type="text/javascript">
        ...
        var r_permission ="{!!$r_permission!!}";
        var roleid = "{!!$role->id!!}";
        d = new dTree('d');
        d.add(0,-1,'权限列表');
        for(let item of myPermissionData){
            if((','+r_permission+',').indexOf(','+item['id']+',') == -1){
                d.add(item['id'],item['pid'],'permission',item['id'],item['name']);
            }else{
                d.add(item['id'],item['pid'],'permission',item['id'],item['name'],true,false);
            }
