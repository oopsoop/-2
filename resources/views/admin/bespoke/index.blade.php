...
@section('content')
    ...
    <p>
        {{--<a href="{{ url('admin/bespokes/create') }}" class="btn btn-success">{{$showdata['app_add_new']}}</a>--}}
    </p>
    <form id="" action="/admin/bespokes/index" method="get" class="form-inline">
                ...
                <tbody>
                    @if ($showdata['data']->count() > 0)
                        @foreach ($showdata['data'] as $proj)
                            <tr>
                                <td>{{ $proj->name }}</td>
                                <td>{{ $proj->gender == 1?'男':'女' }}</td>
                                <td>{{ $proj->age }}</td>
                                <td>{{ $proj->mobile }}</td>
                                <td>{{ $proj->course['course_name'] }}</td>
                                <td>{{ $proj->schedule_id }}</td>
                                <td>{{ $proj->member_level }}</td>

        <td>
            <a href="#" class="up">上移</a> <a href="#" class="down">下移</a> <a href="#" class="top">置顶</a>

            {{--<a href="{{ url('admin/bespokes/update',[$proj->id]) }}" _class="btn btn-xs btn-info"><i class="fa fa-pencil text-success text" style="font-size: 15px;"></i>{{$showdata['app_edit']}}</a>--}}
...
{{--            <a href="{{ url('admin/bespokes/show_course',[$proj->id]) }}" _class="btn btn-xs btn-info"><i class="fa fa-pencil text-success text" style="font-size: 15px;"></i>课程详情</a>--}}
    ...
    <script>
          function del(id) {
              layer.confirm('您确定要删除吗？', {
                  ...
                  $.post("/admin/bespokes/destroy/"+id,{'_method':'delete','_token':"{{csrf_token()}}"},function (data) {
                      if(data.statu==1){
                          layer.msg(data.msg);
