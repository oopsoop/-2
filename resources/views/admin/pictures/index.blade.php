...
@section('content')
    ...
    <p>
        <a href="{{ url('admin/pictures/create') }}" class="btn btn-success">{{$showdata['app_add_new']}}</a>
    </p>
    <form id="" action="/admin/pictures/index" method="get" class="form-inline">
...
{{--                                <td>{{ $proj->user_phone }}</td>--}}
                                ...
                                <td>
                                    ...
                                    <a href="{{ url('admin/pictures/update',[$proj->id]) }}" _class="btn btn-xs btn-info"><i class="fa fa-pencil text-success text" style="font-size: 15px;"></i>{{$showdata['app_edit']}}</a>
{{--                                    <a href="{{ url('admin/pictures/chakan',[$proj->id]) }}" _class="btn btn-xs btn-info"><i class="fa fa-pencil text-success text" style="font-size: 15px;"></i>{{$showdata['app_chakan']}}</a>--}}
    ...
    <script>
          function del(id) {
              layer.confirm('您确定要删除吗？', {
                  ...
                  $.post("/admin/pictures/destroy/"+id,{'_method':'delete','_token':"{{csrf_token()}}"},function (data) {
                      if(data.statu==1){
                          layer.msg(data.msg);
