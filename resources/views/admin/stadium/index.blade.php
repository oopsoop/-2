...
@section('content')
    ...
    <p>
        <a href="{{ url('admin/stadiums/create') }}" class="btn btn-success">{{$showdata['app_add_new']}}</a>
    </p>
    <form id="" action="/admin/stadiums/index" method="get" class="form-inline">
                ...
                <tbody>
                    @if ($showdata['data']->count() > 0)
                        @foreach ($showdata['data'] as $proj)
                            <tr>
                                <td>{{ $proj->stadium_name }}</td>
                                <td>{{ $proj->district }}</td>
{{--                                <td>{{ $proj->xi['status_name'] }}</td>--}}
{{--                                <td>{{ $proj->no['status_name']?$proj->no['status_name']:'' }}</td>--}}
                                <td><img src = "{{  $proj->stadium_pic }}" altno = "" style="height: 44px;width:80px;"></td>
                                <td>{{ $proj->created_at }}</td>
                                <td>
                                    <a href="#" class="up">上移</a> <a href="#" class="down">下移</a> <a href="#" class="top">置顶</a>

                                    <a href="{{ url('admin/stadiums/update',[$proj->id]) }}" _class="btn btn-xs btn-info"><i class="fa fa-pencil text-success text" style="font-size: 15px;"></i>{{$showdata['app_edit']}}</a>
{{--                                    <a href="{{ url('admin/stadiums/chakan',[$proj->id]) }}" _class="btn btn-xs btn-info"><i class="fa fa-pencil text-success text" style="font-size: 15px;"></i>{{$showdata['app_chakan']}}</a>--}}
                                    ...
                                    <a href="{{ url('admin/stadiums/gqrcode',[$proj->id]) }}"  _class="btn btn-xs btn-info"><i class="fa fa-times text-danger text" style="font-size: 15px;"></i>生成二维码</a>
                                </td>
                            </tr>
                        @endforeach
                    @else
    ...
    <script>
          function del(id) {
              layer.confirm('您确定要删除吗？', {
                  ...
                  $.post("/admin/stadiums/destroy/"+id,{'_method':'delete','_token':"{{csrf_token()}}"},function (data) {
                      if(data.statu==1){
                          layer.msg(data.msg);
