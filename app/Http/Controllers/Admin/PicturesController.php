...
class PicturesController extends Controller
{
    ...
    {
//        dd(Picture::find(9)->status_name->status_name);
...
//        $query = Picture::where(function($query) use ($kstime,$jstime,$status){
        $query = Picture::where(function($query) use ($kstime,$jstime){
//            if ($status != "全部") {
...
//            'status' => $showdata['datac']['status']
        ...
        return view('admin.pictures.index', compact('showdata','input'));
    }
...
//        }
        ...
        if ($request->input('is') == 1) {
            ...
            $rules = [
                'title' => 'required|min:2|max:191|unique:pictures,title',
...
//         return redirect('admin/prompt')->with(['message'=>'登录成功，即将跳转到后台首页','url' =>'/admin/latestpolicy/create', 'jumpTime'=>3,'status'=>'success'])->withCookie($cookie);
            return redirect('admin/prompt')->with(['message' => '添加成功', 'url' => '/admin/pictures/index', 'jumpTime' => 3, 'status' => 'success']);
        }
        //状态表查询
...
//        $showdata['status_t'] = $this->CreateSelect2Data(\App\Status_t::orderBy('id','asc')->where('ty','4')->get(),'status_name','add');
        ...
        return view('admin.pictures.create', compact('showdata'));
    }
...
//            return redirect('admin/prompt')->with(['message'=>'修改成功','url' =>'/admin/latestpolicy', 'jumpTime'=>3,'status'=>'success']);
        ...
        $corp = Picture::findOrFail($id);

        $showdata = $this->showdata;
        $showdata['page-title'] = '';
        return view('admin.pictures.edit', compact('corp', 'showdata'));
    }
...
//                'district.required' => '所属区县必须填写'
            ...
            $validator = Validator::make($request->all(), $rules, $messages);
            if ($validator->fails()) {
                return redirect('admin/pictures/update/' . $id)
                    ->withErrors($validator)
                    ->withInput($request->all());
            }
...
//            $diffKey = ['title','status_id','sortby','cnt'];
            ...
            if (Picture::findOrFail($id)->update($input) === true) {
//                $vg = 0;
//                foreach ($diffKey as $item){
//
//                    if(Picture::findOrFail($id)->$item == $r[$item]){
//                        $r[$item] = null;
//                    }else{
//
//                        $vg = $vg + 1;
...
//                    if($is_create_ok){
                        return redirect('admin/prompt')->with(['message' => '修改成功', 'url' => '/admin/pictures/index', 'jumpTime' => 3, 'status' => 'success']);
//
//                    }
//                }
...
//
        ...
        return view('admin.pictures.chakan', compact('corp', 'showdata','history','addhistory'));
    }
    /**
