...
class Picture extends Model
{
    ...
    protected $fillable = ['id','status_id,','pic','title','pictures_type','region_type','company_info','remarks','intro','doc_pic','content'];
...
//    public function getpicturesById($pageNo=1, $pageSize=10, $cate='',$province_id='',$city_id='')
//    {
...
//            $query = $this->_db->prepare("select `id`, `title`,`contents`,`author`,`cate`,`ctime`,`mtime`,`status` from `art` where `status`=? order by `ctime` desc limit ?,?  ");
            $ret = \DB::table('pictures')->skip($start)->take($pageSize)->get();
            $count =count(\DB::table('pictures')->get());
//            $query = $this->_db->prepare("select `id`, `title`,`contents`,`author`,`cate`,`ctime`,`mtime`,`status` from `art` where `status`=? order by `ctime` desc limit ?,?  ");
//        }
//        else {
...
//            $ret = \DB::table('pictures')->whereRaw('cate_id = '.$cate)->skip($start)->take($pageSize)->get();
//            $count =count(\DB::table('pictures')->whereRaw('cate_id = '.$cate)->get());
//        }
